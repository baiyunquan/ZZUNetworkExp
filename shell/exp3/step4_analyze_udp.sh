...
# 只分析本实验会话: 过滤 udp.port==4499，避免混入无关 UDP 流量。
echo "==> (1) 筛选 UDP 报文（等价显示过滤器 udp.port == 4499）"
tshark -r "$PCAP" -Y "udp.port == 4499" -c 10

echo ""
echo "==> (2) 提取 UDP 首部字段（源端口/目的端口/长度/校验和）"
tshark -r "$PCAP" -Y "udp.port == 4499" -T fields \
...
cat <<'EOF'
   ...
   - 校验和(2B): 覆盖伪首部+首部+数据，用于接收方差错检测
     （RFC 768 中 IPv4 下可选，全 0 表示不计算；但 Linux 协议栈总是计算）
   对比 TCP: 无序号、无确认、无握手 —— 体现无连接、不可靠、尽最大努力交付
EOF

echo ""
echo "==> (4) 校验和验证（checksum.status: good 表示校验通过）"
tshark -r "$PCAP" -Y "udp.port == 4499" -T fields -e udp.checksum.status | sort | uniq -c
