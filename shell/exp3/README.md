...
| 4 | `step3_udp_comm.sh` | 步骤3/4/5 | H57C 抓包 + ncat UDP 回显服务端双向通信（自动化，含成败判定退出码；交互式步骤3 方法见脚本末尾注释） |
...
sudo ./create_topology.sh verify        # ping 4 包全通 + traceroute 4 跳 + offload 断言
...
2. **步骤2 verify**：7 个命名空间；两网桥各绑 2 接口；三路由器 `ip_forward=1`；offload 断言全部 `off ✓`；`ping -c 4 192.168.57.254` → `4 received, 0% packet loss`；`traceroute` 输出 **4 跳**：`192.168.56.1(RB) → 192.168.56.246(RA) → RD(192.168.56.254) → 192.168.57.254(H57C)`（TTL=3 的包在 RD 处到期，RD 回 ICMP Time Exceeded）。
3. **offload 检查**：所有 VETH 的 rx/tx-checksumming、gso/gro 均为 **off**（保证校验和/分片由 CPU 计算，Wireshark 才能验证真实校验和）。
4. **步骤5 UDP 通信**：客户端发送一行字符，服务端回显，客户端收到相同内容——双向通信成功（脚本回显验证失败时以非零退出码提示）；pcap 中**无握手/确认报文**（对比 TCP），源/目的端口 4499 ↔ 临时端口。
