...
| 2 | **`create_topology.sh`** | **步骤2（要求实现的脚本）** | 一键 create/verify/destroy：14 命名空间 + 4 网桥 + 13 对 VETH + 全网静态路由 |
...
3. **步骤6**：nping 发送 1 个 1400B 载荷 UDP 报文（IP 总长 1428 > MTU 1000）：
   ...
   - **出接口 pcap**：2 个分片——片1 len=1000/MF=1/offset=0，片2 len=452/MF=0/offset=122（=976/8），两片 `ip.id` 相同、TTL 均减 1、仅片1 带 UDP 头
4. **步骤8**：三条 traceroute 逐跳输出途经路由器，与规划路径一致；各路由器路由表体现**最长前缀匹配**的逐跳转发算法。

## 注意事项

- `nping`/`ncat` 属 `nmap` 包；`routel` 属 iproute2 包。
- 分片偏移按 8 字节对齐：976B 数据（976/8=122），片2 携带剩余 432B。
- traceroute 某跳显示 `* * *` 多为路由器对探测包限速回复，不影响后续跳，属正常。
- GW 命名空间仅用于模拟互联网出口（192.168.99.1），使 RA 的默认路由下一跳可达。
